-- Fix the role system: restore Employee → Manager → Admin hierarchy

-- Step 1: Update the user_role enum to use 'employee' instead of 'user'
ALTER TYPE public.user_role RENAME TO user_role_old;
CREATE TYPE public.user_role AS ENUM ('employee', 'manager', 'admin');

-- Step 2: Update the users table to use the correct enum
...
-- Step 4: Update database functions to use correct role hierarchy
...
-- Step 5: Update handle_new_user function to assign employee role correctly
...
-- Step 6: Update user_has_permission function to work with new role hierarchy
