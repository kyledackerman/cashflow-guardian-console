-- Fix missing user_has_role function and authorization system
...
-- Create new RLS policies for petty cash
...
-- Create policies for garnishment profiles
CREATE POLICY "Users can view garnishment profiles" 
ON public.garnishment_profiles FOR SELECT 
USING (user_has_permission('VIEW_FINANCES'));

CREATE POLICY "Managers can manage garnishment profiles" 
ON public.garnishment_profiles FOR ALL 
USING (user_has_permission('EDIT_TRANSACTIONS'));

-- Create policies for garnishment installments
CREATE POLICY "Users can view garnishment installments" 
ON public.garnishment_installments FOR SELECT 
USING (user_has_permission('VIEW_FINANCES'));

CREATE POLICY "Managers can manage garnishment installments" 
ON public.garnishment_installments FOR ALL 
USING (user_has_permission('EDIT_TRANSACTIONS'));

-- Create policies for garnishment documents
CREATE POLICY "Users can view garnishment documents" 
ON public.garnishment_documents FOR SELECT 
USING (user_has_permission('VIEW_FINANCES'));

CREATE POLICY "Managers can manage garnishment documents" 
ON public.garnishment_documents FOR ALL 
USING (user_has_permission('EDIT_TRANSACTIONS'));

-- Create policies for bulk payment batches
CREATE POLICY "Users can view bulk payment batches" 
ON public.bulk_payment_batches FOR SELECT 
USING (user_has_permission('VIEW_FINANCES'));

CREATE POLICY "Managers can manage bulk payment batches" 
ON public.bulk_payment_batches FOR ALL 
USING (user_has_permission('EDIT_TRANSACTIONS'));

-- Create policies for employee loan withdrawals
CREATE POLICY "Users can view loan withdrawals" 
ON public.employee_loan_withdrawals FOR SELECT 
USING (user_has_permission('VIEW_FINANCES'));

CREATE POLICY "Managers can create loan withdrawals" 
ON public.employee_loan_withdrawals FOR INSERT 
WITH CHECK (user_has_permission('EDIT_TRANSACTIONS'));

CREATE POLICY "Managers can update loan withdrawals" 
ON public.employee_loan_withdrawals FOR UPDATE 
USING (user_has_permission('EDIT_TRANSACTIONS'))
WITH CHECK (user_has_permission('EDIT_TRANSACTIONS'));

-- Create policies for employee loan repayments
CREATE POLICY "Users can view loan repayments" 
ON public.employee_loan_repayments FOR SELECT 
USING (user_has_permission('VIEW_FINANCES'));

CREATE POLICY "Managers can manage loan repayments" 
ON public.employee_loan_repayments FOR ALL 
USING (user_has_permission('EDIT_TRANSACTIONS'));

-- Create policies for employee loan requests
CREATE POLICY "Users can view loan requests" 
ON public.employee_loan_requests FOR SELECT 
USING (user_has_permission('VIEW_FINANCES'));

CREATE POLICY "All users can create loan requests" 
ON public.employee_loan_requests FOR INSERT 
WITH CHECK (true);

CREATE POLICY "Managers can update loan requests" 
ON public.employee_loan_requests FOR UPDATE 
